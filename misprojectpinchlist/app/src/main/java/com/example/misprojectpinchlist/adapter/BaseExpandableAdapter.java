package com.example.misprojectpinchlist.adapter;
...
import com.example.misprojectpinchlist.viewholder.AdapterItemUtil;
import com.example.misprojectpinchlist.viewholder.BaseAdapterItem;
...
public abstract class BaseExpandableAdapter extends RecyclerView.Adapter implements AbstractExpandableAdapterItem.ParentListItemExpandCollapseListener {
    ...
    private void checkDefaultExpand() {
        ...
        while (iterator.hasNext()) {
            ...
            if (next instanceof ExpandableListItem) {
                ...
                if (expandableListItem.isExpanded()) {
                    List<?> childItemList = expandableListItem.getChildItemList();//获得子item
                    if (childItemList != null && !childItemList.isEmpty()) {
                        childArrayMap.put(next, childItemList);//可扩展的统一放到这里
                    }
                }
            }
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            int indexOf = mDataList.indexOf(o);//第一次出现o的位置
            mDataList.addAll(indexOf + 1, objects);//将子objects中所有item的到父item(o)位置的下一个位置起始放置。
        }
